...
<!-- NOV2017 ADDED ERROR test X=1 1.kysysmys incorrect -->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/" mode="kysymys11">
		...
		<!-- ADDED ERROR X=1 (po. "//main_image/@*" ) -->
		<xsl:for-each select="//main_image/@language">
			<xsl:value-of select="."/>
			<xsl:text>&#xA;</xsl:text>
		</xsl:for-each>
	</xsl:template>
	...
	<xsl:template match="/" mode="kysymys13">
		...
		<xsl:for-each select="/ancient_wonders/wonder[./history/year_destroyed/@era='BC']/.">
